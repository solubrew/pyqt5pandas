...
<(META)>:  #																	||
	docid: ''  #							||
	name: Moonbags Nchnated Token Widgets Python Document#						||
	description: >  #															||
...
def read_sql(sql, con, filePath, index_col=None, coerce_float=True,
	...
	df = pandas.read_sql(sql, con, index_col, coerce_float, params, parse_dates, columns, chunksize=None)
...
class DataFrameModel(QtCore.QAbstractTableModel):
	...
	def filePath(self):
		...
		:return: pyqt5pandas.models.DataFrameModel._filepath"""
	...
	def data(self, index, role=Qt.DisplayRole):
		...
		def convertValue(row, col, columnDtype):
			...
			if columnDtype == object:
				...
				if log: print('Col', col, 'Row', row)
				if log: print('item', self._dataFrame.loc[row, col])
		...
		if role == Qt.DisplayRole:
			...
			if columnDtype == numpy.bool:
				result = self._dataFrame.iloc[row, col]
			else:
	...
	def setFilter(self, search):
		...
		Args:
			search(pyqt5pandas.DataSearch): data search object to use.
		...
		if not isinstance(search, DataSearch):
			raise TypeError('The given parameter must an `pyqt5pandas.DataSearch` object')
	...
	def columnDtypeModel(self):
		...
		:return:
			pyqt5pandas.models.ColumnDtypeModel
		"""
	...
	def addDataFrameColumn(self, columnName, dtype=str, defaultValue=None):
		...
		:param columnName: str
			name of the column.
		:param dtype: pyqt5pandas.models.SupportedDtypes option
